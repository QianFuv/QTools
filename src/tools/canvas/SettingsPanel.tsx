...
import type { CanvasCourse, CanvasSettings } from "./types";

interface SettingsPanelProps {
  settings: CanvasSettings | null;
  courses: CanvasCourse[];
  onSave: (settings: CanvasSettings) => Promise<void>;
}

export default function SettingsPanel(props: SettingsPanelProps) {
  const [apiUrl, setApiUrl] = createSignal(props.settings?.api_url ?? "https://canvas.uts.edu.au");
  const [apiToken, setApiToken] = createSignal(props.settings?.api_token ?? "");
  const [cacheTtl, setCacheTtl] = createSignal(props.settings?.cache_ttl_minutes ?? 30);
  const [ignoredIds, setIgnoredIds] = createSignal<Set<number>>(
    new Set(props.settings?.ignored_course_ids ?? []),
  );
  ...
  const inputClass =
    "w-full rounded-lg border border-qtools-300 bg-white px-3 py-2 text-sm text-qtools-900 outline-none transition-colors focus:border-qtools-500 focus:ring-1 focus:ring-qtools-500 dark:border-qtools-600 dark:bg-qtools-800 dark:text-qtools-50 dark:focus:border-qtools-400 dark:focus:ring-qtools-400";

  const labelClass = "block text-sm font-medium text-qtools-700 dark:text-qtools-200";

  return (
    <div class="mx-auto max-w-xl p-6">
      <h2 class="mb-6 text-lg font-bold text-qtools-900 dark:text-qtools-50">Canvas Settings</h2>
      ...
      <div class="flex flex-col gap-5">
        ...
        <Show when={props.courses.length > 0}>
          <div>
            ...
            <div class="mt-1 flex flex-col gap-1 rounded-lg border border-qtools-200 bg-qtools-50 p-2 dark:border-qtools-700 dark:bg-qtools-900">
